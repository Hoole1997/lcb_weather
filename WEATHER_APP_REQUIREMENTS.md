...
- [x] ✅ 所有主要按钮有清晰点击反馈。
- [x] ✅ 文本在小屏幕下不截断关键内容。
- [x] ✅ 主天气页视觉层级清晰，当前天气优先级最高。
- [x] ✅ 城市管理页卡片密度适中，适合频繁切换城市。
- [x] ✅ 添加城市页搜索状态明确。
- [x] ✅ 设置页使用清晰分组。
- [x] ✅ 空态、错误态、加载态文案清晰。
- [x] ✅ 避免过度复杂动画，保留轻量过渡。
...
```text
已完成 UI/UX 小范围完善。WeatherTheme 支持浅色/深色/跟随系统，所有页面使用 MaterialTheme 色板与系统栏 padding；主要操作使用 Button/FAB/IconButton/AssistChip，点击反馈由 Material 组件提供。主天气页保持当前天气 Hero 最高层级，错误态无旧数据时不再显示无意义 loading spinner；城市管理页移除无效间距并保持紧凑卡片；添加城市页经纬度使用稳定格式并保留明确加载/空/错状态；设置页使用单位、外观、其他分组。WeatherNavGraph 移除了遗留占位页面代码，后续维护入口更清晰。验证命令：./gradlew :app:compileLocalDebugKotlin，结果 BUILD SUCCESSFUL。
```
...
2026-05-12：清理 App 模块国际化资源中的 app_name。默认 app/src/main/res/values/strings.xml 保留 <string name="app_name" translatable="false">天气</string>，values-en、values-zh-rTW、values-ja、values-ko、values-fr、values-de、values-es、values-pt、values-it、values-ru 中移除 app_name，让所有语言统一回退默认应用名。验证命令：./gradlew :app:compileLocalDebugKotlin，结果 BUILD SUCCESSFUL。
```
